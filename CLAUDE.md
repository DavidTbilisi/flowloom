...
- **The CLI (`src/cli.ts`) is a third headless consumer**, alongside the Node tests and the worker. It imports *only* the `src/engine`/`src/lang` barrels (never `src/ui`) and runs the same parse → compile → `simulateAsync` → render pipeline as the browser, so the numbers match by construction. It builds separately from the web app: `tsconfig.cli.json` emits `cli.ts` plus the reachable engine/lang graph to `dist-cli/` as Node ESM (the `.js` import extensions already in source are what make that resolve at runtime). `run`/`loops`/`check` mirror studio features; `--set k=v` rewrites a param/stock-init/sim-setting on the parsed `Model` before running (an AST edit, keeping the text-canonical spirit). If you reach into `src/ui` from the engine/lang, the web app may survive but `npm run build:cli` breaks.
